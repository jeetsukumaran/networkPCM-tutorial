...
using PhyloMakie        # for plotting networks: native Julia, no R dependency
...
display(res) # subfigure (a): each display() call becomes one fig-subcap panel
Makie.data_limits(res.plot) # default x/y limits for this network
for nodenumber in [-16, -17,-6,-4, -19,-21]
  PhyloNetworks.rotate!(net_snaq, nodenumber) # qualified: Makie also exports rotate!
end
plot(net_snaq, xlim=[0,16]) # subfigure (b): extend limits to show full taxon names
```
...
(as shown [previously](topic3-averagedistances.qmd) on a small sample of genes).
The results (using all genes) were saved in file
`data/polemonium_geneticpairwisedistances.csv`,
which we read below.
...
fmin, ret # if fmin increased slightly: score got worse
```
```{julia}
fmin, xmin, ret = calibratefrompairwisedistances!(net, avD, taxa,
        forceMinorLength0=false, NLoptMethod=:LD_MMA);
...
display(plot(net, useedgelength=true, xlim=[1,2.5], showgamma=true)) # subfigure (a)
plot(net, useedgelength=true, xlim=[1,2.5], style=:majortree, arrowlen=0.07) # subfigure (b)
```

Finally, let's save our calibrated network to a file:
```{julia}
writenewick(net,"data/polemonium_network_calibrated.phy")
```
